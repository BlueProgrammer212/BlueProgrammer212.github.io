		...
		if (auth2.isSignedIn.get()) {
			...
			if ("id" in params_) firestore.collection("profiles").doc(params_.id).get().then((a) => {
				if (!a.exists && params_.id === getCookie("pf_id")) {
					...
					firestore.collection("profiles").doc(params_.id).set({
						id: getCookie("pf_id"),
						name: getCookie("pf_name"),
				...
				if (a.data().id == getCookie("pf_id")) {
						...
						firestore.collection("profiles").doc(a.data().pending_friend_requests[i].profile_id).get().then((nf) => {
							...
							document.getElementsByClassName("accept_req")[i].onclick = async function() {
								if ("id" in params_ && params_.id == getCookie("pf_id")) await firestore.collection("profiles").doc(params_.id)
									.update({
										"friends": firebase.firestore.FieldValue["arrayUnion"]({
											"profile_id": a.data().
											pending_friend_requests[i].profile_id,
											"verified": true
										}),
										"pending_friend_requests": firebase.firestore.FieldValue["arrayRemove"]({
											"profile_id": a.data().pending_friend_requests[i].profile_id
										})
									})
								...
								if ("profile_id" in a.data().pending_friend_requests[i]) await firestore.collection("profiles")
									.doc(a.data().pending_friend_requests[i].profile_id).update({
										"friends": firebase.firestore.FieldValue["arrayUnion"]
											({
												"profile_id": params_.id,
												"verified": true
											})
									}).then((a) => {
										console.log(a)
					...
					document.getElementById("save_desc").onclick = function() {
						firestore.collection("profiles").doc(params_.id).update({
							description: document.getElementById("aboutMeEdit").value
						})
					}
				} else {
					let info_, info;
					void async function init_information() {
						...
						if (info_.data().friends.some(function(x) {
								return x.profile_id === params_.id
							})) {
							...
							document.getElementById("AddFriend").addEventListener("click", async () => {
								if (!info["data"]().pending_friend_requests.some((x) => {
										return x.profile_id === getCookie("pf_id")
									})) {
									firestore.collection("profiles").doc(params_.id).update({
										pending_friend_requests: firebase.firestore.FieldValue.arrayUnion({
